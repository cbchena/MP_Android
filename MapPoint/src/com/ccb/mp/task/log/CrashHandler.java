...
/**
 * UncaughtException处理类,当程序发生Uncaught异常的时候,有该类来接管程序,并记录发送错误报告. 2015/6/25 16:34
 * @author user
 */
public class CrashHandler implements UncaughtExceptionHandler {

    private static Logger logger = LoggerFactory
            .getLogger(CrashHandler.class); // 日志对象

    //系统默认的UncaughtException处理类
    private UncaughtExceptionHandler _mDefaultHandler;
    ...
    /**
     * 当UncaughtException发生时会转入该函数来处理 2015/6/25 16:35
     */
    @Override
    ...
    /**
     * 自定义错误处理,收集错误信息 发送错误报告等操作均在此完成. 2015/6/25 16:34
     * @param ex
    ...
    private boolean _handleException(Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                Toast.makeText(_mContext, "很抱歉,应用程序出现异常,即将退出。", Toast.LENGTH_LONG).show();
                Looper.loop();
            }
        }.start();
    ...
    /**
     * 收集设备参数信息 2015/6/25 16:34
     * @param ctx 上下文环境
     */
    private void _collectDeviceInfo(Context ctx) {
        logger.debug("Collect device information.");
        try {
    ...
    /**
     * 保存错误信息到文件中 2015/6/25 16:34
     * @param ex
     * @return	返回文件名称,便于将文件传送到服务器
     */
    private String _saveCrashInfo2File(Throwable ex) {
        logger.debug("Save crash information to local file.");
        ...
        try {
            ...
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                String path = "/sdcard/mpoint/crash/";
